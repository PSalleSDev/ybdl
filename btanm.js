        ...
        modules.request(0).get(url + "/download", (err, response, a) => {
            ...
            srcs.forEach(item => {
                ...
                modules.request(0).get(href, (err, response, b) => {
                    ...
                    modules.request(0).get(`https://download.betteranime.net/_next/data/${key}/${href.split("/")[3]}.json`, (err, response, c) => {
                       Append(quality, JSON.parse(c).pageProps.anime.path, srcs)
                    })
                })
            })
        })
